...
void loadTextures()
{
   ...
   // Load brick texture
	...
	glBindTexture(GL_TEXTURE1, texID);
	loadTGA("Moss.tga");
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
}
...
void initialise()
{
	...
	texLoc = glGetUniformLocation(program, "tSampler2");  //tSampler1 is a uniform variable of type Sampler2D in the fragment shader
	glUniform1i(texLoc, 1);    //Assign a value 0 to the variable tSampler1 to specify that it should use the texture from unit 0.
